"""
crypto_bot_v3.py — Upgraded ETH/BTC/SOL Up/Down 5min Paper Trading Bot
Phase 1 + Phase 2 + Phase 3 (Strategy 6 Max-Edge + Dump-and-Hedge)

Phase 1:
  - Binance WebSocket price feed for the critical last 90 seconds
  - Hard "too close to the line" filter
  - Better logging + session / daily P&L summary
  - Cleaner resolution tracking

Phase 2:
  - Multi-asset: BTC, ETH, SOL
  - Dynamic position sizing based on confidence + edge
  - Simulated auto-redeem of winning paper positions
  - Per-asset and overall performance stats

Phase 3 (NEW):
  - Strategy 6 Max-Edge: strict high-confidence filter
      conf >= 56% AND not (Up + px >= 0.99)
      Size boosts: Down 2.0x | Value (px<=0.97) 2.5x | Late (<=35s) 1.5x
  - Dump-and-Hedge: detect sharp one-sided PM price dumps and lock profit
      when combined Up+Down cost is attractive
  - Core directional still runs as fallback (with improved min conf)

Strategy priority:
  1. Strategy 6 Max-Edge (highest quality, lowest frequency)
  2. Dump-and-Hedge (semi-arb on sharp moves)
  3. Core directional (improved conf floor)

Usage:
    python crypto_bot_v3.py                  # paper, $3000 start
    python crypto_bot_v3.py --amount 15
    python crypto_bot_v3.py --balance 5000
"""
...
# Phase 3: raised confidence floor for core + Strategy 6
MIN_CONFIDENCE = 0.22          # still used for very basic filter
MIN_CONFIDENCE_CORE = 0.56     # Strategy 6 / improved core floor
...
# Strategy 6 size multipliers
S6_DOWN_MULT     = 2.0
S6_VALUE_MULT    = 2.5      # px <= 0.97
S6_LATE_MULT     = 1.5      # left <= 35s
S6_DANGEROUS_PX  = 0.99     # skip Up at/above this

# Dump-and-Hedge parameters
DUMP_THRESHOLD      = 0.18   # 18% drop triggers
HEDGE_SUM_TARGET    = 0.96   # only full hedge when combined <= this
...
class PriceCache:
    def __init__(self):
        ...
        self._symbols = list(BINANCE_SYMBOLS.values())
    ...
    def start_ws(self):
        if not HAS_WS or self._ws_running:
            return
        ...
        log("[WS] Binance trade stream started")
...
# ─── Dump-and-Hedge price history ──────────────────────────────────────────────
# Stores recent (timestamp, up_px, down_px) per asset for dump detection
...
def get_clob_price(token_id: str) -> float:
    try:
        r = requests.get(f"{CLOB_API}/midpoint", params={"token_id": token_id}, timeout=3)
        r.raise_for_status()
...
def strategy_6_max_edge(
    ...
    """
    Strict high-edge layer.
    Only takes conf >= 56% AND not (Up + px >= 0.99).
    Applies size boosts for Down / Value / Late entries.
    """
    if confidence < MIN_CONFIDENCE_STRICT:
        return None

    if direction == "Up" and px >= S6_DANGEROUS_PX:
        return None  # skip the historically dangerous bucket

    # Size multipliers (highest applicable wins)
    size_mult = 1.0
...
def check_dump_and_hedge(
    ...
    """
    Detect sharp one-sided dumps in PM prices and attempt to lock profit
    when combined cost becomes attractive.
    """
    ...
    if up_dump and not down_dump:
        ...
        # both dumped – skip to avoid noise
        return None

    combined = dumped_px + hedge_px

    if combined <= HEDGE_SUM_TARGET:
        # Attractive combined cost → take both legs (true dump-and-hedge)
    ...
    # Only dumped leg for now (can be extended with pending-hedge logic later)
...
class CryptoBot:
    def __init__(self, paper: bool, dry_run: bool, amount: float, paper_balance: float):
        ...
        log("=" * 70)
        log(f"Crypto Up/Down Bot v3 | {mode} | base ${amount}/trade | balance ${paper_balance:.2f}")
        log(f"Assets: {', '.join(MARKETS.values())}")
        log(f"Entry: {ENTRY_SECONDS_MIN}-{ENTRY_SECONDS_MAX}s | "
            f"Price min BTC={PRICE_MIN['BTC']} ETH/SOL={PRICE_MIN['ETH']} max={PRICE_MAX}")
        log(f"Delta skip={DELTA_SKIP*100:.3f}% hard={DELTA_HARD*100:.3f}% | "
            f"Min conf core={MIN_CONFIDENCE_CORE*100:.0f}%")
        log(f"Dynamic sizing: {MIN_AMOUNT_MULT:.1f}x – {MAX_AMOUNT_MULT:.1f}x base | "
            f"WS={'ON' if HAS_WS else 'OFF (REST only)'}")
        log("─" * 70)
        log("Phase 3 Strategies enabled:")
        log(f"  • Strategy 6 Max-Edge  (conf≥{MIN_CONFIDENCE_STRICT*100:.0f}% + not Up@≥{S6_DANGEROUS_PX})")
        log(f"      size: Down {S6_DOWN_MULT}x | Value(px≤0.97) {S6_VALUE_MULT}x | Late(≤35s) {S6_LATE_MULT}x")
        log(f"  • Dump-and-Hedge       (dump≥{DUMP_THRESHOLD*100:.0f}% → hedge if sum≤{HEDGE_SUM_TARGET})")
        log("=" * 70)
    ...
    def _cycle(self):
        ...
        while not self._shutdown:
            ...
            def fetch_all(prefix):
                ...
                # Refresh mid prices if possible
                clob = get_clob_price(market["winner_token"])
                if clob > 0:
                    market["winner_price"] = clob
                # Also try to get both sides for dump detection
            ...
            with ThreadPoolExecutor(max_workers=len(pending)) as ex:
                futures = {ex.submit(fetch_all, p): p for p in pending}
                results = []
            ...
            for prefix, market, ta in results:
                ...
                # Keep PM price history for dump detection
    ...
    def _evaluate_entry(self, market, ta, seconds_left, entered_slugs):
        ...
        # Basic hard filters that apply to all strategies
        if price < price_min:
        ...
        # ── Priority 1: Strategy 6 Max-Edge ──────────────────────────────────
        ...
        if dah:
            if dah["action"] == "ENTER_BOTH":
                ...
                for leg in dah["legs"]:
                    # Temporarily override side/price for each leg
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Upgraded 5m crypto paper bot v3 (BTC/ETH/SOL)")
